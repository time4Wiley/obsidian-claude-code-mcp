...
class McpTester {
    ...
    async runHttpTests() {
        ...
        for (const test of tests) {
            try {
                ...
                this.log(`✅ ${test.name}: ${result.statusCode} - ${JSON.stringify(result.response)}`);
                this.testResults.push({ test: test.name, endpoint: '/mcp', success: true, result });
            } catch (error) {
                this.log(`❌ ${test.name}: ${error.message}`, 'ERROR');
                this.testResults.push({ test: test.name, endpoint: '/mcp', success: false, error: error.message });
            }
        ...
        for (const test of tests) {
            try {
                const result = await this.testHttpPost('/messages', test.request);
                this.log(`✅ ${test.name}: ${result.statusCode} - ${JSON.stringify(result.response)}`);
                this.testResults.push({ test: test.name, endpoint: '/messages', success: true, result });
            } catch (error) {
                this.log(`❌ ${test.name}: ${error.message}`, 'ERROR');
                this.testResults.push({ test: test.name, endpoint: '/messages', success: false, error: error.message });
            }
            await this.delay(100);
        }
    }
    ...
    async runSseTests() {
        ...
        for (const endpoint of sseEndpoints) {
            try {
                ...
                this.log(`✅ SSE ${endpoint}: Received ${events.length} events`);
                this.testResults.push({ test: 'SSE Connection', endpoint, success: true, events });
            } catch (error) {
                this.log(`❌ SSE ${endpoint}: ${error.message}`, 'ERROR');
                this.testResults.push({ test: 'SSE Connection', endpoint, success: false, error: error.message });
            }
        }
    }
                ...
                if (lockFiles.length > 0) {
                    ...
                    return new Promise((resolve) => {
                        ...
                        ws.on('message', (data) => {
                            ...
                            this.log(`WebSocket response: ${JSON.stringify(response)}`);
                            ws.close();
                            resolve(true);
                        });
    ...
    async runAllTests() {
        ...
        try {
            ...
            this.log('❌ Server not responding. Make sure Obsidian plugin is running!', 'ERROR');
            return;
        }
